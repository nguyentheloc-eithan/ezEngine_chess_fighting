...
void Sherlock::move()
{
    // cout << "MOVING CHECK GETNEXT-sherlock " << endl;
    // cout << this->getNextPosition().getRow() << " " << this->getNextPosition().getCol() << " " << this->exp << endl;
    // if (this->getNextPosition().getRow() != -1 && this->getNextPosition().getCol() != -1 && this->getEXP() != 0)
    // {
    //     this->pos = this->getNextPosition();
    //     this->index_moving_rule++;
    // }
    Position nextPosSherlock = this->getNextPosition();
    if (nextPosSherlock.getRow() != -1 && nextPosSherlock.getCol() != -1 && this->getEXP() != 0)
    {
        this->pos = nextPosSherlock;
        this->index_moving_rule++;
    }
    else
    {
...
void Watson::move()
{
    Position nextPosWatson = this->getNextPosition();
    if (nextPosWatson.getRow() != -1 && nextPosWatson.getCol() != -1 && this->getEXP() != 0)
    {
        this->pos = nextPosWatson;
        this->index_moving_rule++;
    }
    else
    {
...
bool Watson::meet(RobotW *robotW)
{
    if (this->getBag()->checkItem(ItemType::PASSING_CARD) && this->hp % 2 == 0)
    {
        BaseItem *passingCard = this->bag->get(ItemType::PASSING_CARD);
